...
class TestPercentileTable:
    ...
    def test_non_numeric_raises(self, cat_series):
        with pytest.raises(TypeError):
            percentile_table(cat_series)


class TestValueCounts:
    def test_returns_dataframe(self, cat_series):
        result = value_counts(cat_series)
        assert isinstance(result, pd.DataFrame)
    ...
    def test_top_k_limits_rows(self, cat_series):
        result = value_counts(cat_series, top_k=2)
        assert len(result) == 2
...
class TestBarCount:
    def test_returns_figure(self, cat_series):
        fig = bar_count(cat_series)
        assert isinstance(fig, go.Figure)

    def test_horizontal_orientation(self, cat_series):
        fig = bar_count(cat_series, horizontal=True)
        assert isinstance(fig, go.Figure)
...
class TestPieChart:
    def test_returns_figure(self, cat_series):
        fig = pie_chart(cat_series)
        assert isinstance(fig, go.Figure)

    def test_top_k_groups_others(self, cat_series):
        # cat_series has 4 unique values; top_k=2 → "Others" label present
        fig = pie_chart(cat_series, top_k=2)
